...
与 `.c/.cpp` 文件一致, `.h/.hpp` 文件也请全部放到 `src/` 目录中。一种推荐的做法是，将同一个模块的代码文件: `.h/.hpp/.c/.cpp` 放在同一个目录下。


我们在 `CMakeLists.txt` 下编写了一个宏函数 `collect_head_dirs`, 它将自动遍历 `src/` 及其所有递归子目录。如果某个目录内含 `.h/.hpp` 文件。这个目录就会被收集到一个 `list` 中。最后,这个 `list` 会被自动添加进 include 路径列表。

所以，当你在 `src/` 及其递归子目录 下的任何位置添加 `.h/.hpp` 文件时，cmake 都能自动帮你 include 好。

比如说，你在目录 `.../src/a/b/c/` 下，新建文件 `koko.h`, 那么，你就能在 `src/` 目录下的任一 代码文件内，inlcude 到它：
...
当然，这种实现存在一个代价，就是强制要求你在 `src/` 目录下的所有 `.h/.hpp` 文件，都不能重名。

最后，如果你不喜欢这种实现，你可以在 `CMakeLists.txt` 中放弃这个 宏函数 的使用，通过手动添加 include 路径的方式，来精细地管理 include 路径列表。具体请搜索 cmake: `target_include_directories` 的用法。

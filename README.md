...
原则上，你可以在目录 `src` 下创建任意 子目录（递归）或文件。在目前的 cmake 设置中，所以位于目录 `src` （及其递归子目录）下的 **.cpp**， 都会被自动查找到并编译，正如 文件 `CMakeLists.txt` 中所写：
...
整体上，我推荐大家使用第一种写法，毕竟，在一个项目中，添加目录并不是一件非常频繁的事。尤其是进入到项目中后期。


当然，一定还存在更加高效地，自动包含所有 `.h` 文件的 **cmake** 写法。如果有，还请大家告诉我（ 我的 cmake 玩得太菜了hhhh...
...
## sysconfig 目录
在根目录中，我们发现一个奇怪的目录 `sysconfig/`，它的体内包含一个名为 `SysConfig.h.in` 的文件。这个文件是提供给 cmake，生成平台相关信息 用的。在编译过程中，cmake 会根据当前信息，在目录 `.../publish/sysconfig/` 中，自动创建一个文件 `SysConfig.h`。
...
通过这个机制，你可以在自己的程序代码中，先 `#include "SysConfig.h"`，然后检查是否存在某个宏，来确定自己所处的 操作系统类型。对于那些跨平台的，且依赖平台API 的程序来说，这会是个有用的功能。（你可以编写数个的分支代码，来处理各个平台） 
